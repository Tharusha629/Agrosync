...
function AllFarmers() {
  ...
  const generatePDF = () => {
    ...
    // Keep existing logo code
    const logoWidth = 40;
    const logoHeight = 40;
    const pageWidth = doc.internal.pageSize.getWidth();
    const logoX = (pageWidth - logoWidth) / 2;
    
    doc.addImage(COMPANY_LOGO, 'PNG', logoX, 10, logoWidth, logoHeight);
    
    // Enhanced title styling
    doc.setFontSize(22);
    doc.setTextColor(44, 62, 80); // Dark blue color
    doc.setFont('helvetica', 'bold');
    doc.text("Farmers Report", pageWidth/2, 60, { align: 'center' });
    ...
    doc.text(`Generated on: ${date} at ${time}`, pageWidth/2, 67, { align: 'center' });
