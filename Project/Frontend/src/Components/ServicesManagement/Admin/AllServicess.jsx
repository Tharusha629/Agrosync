...
function AllServicess() {
  ...
  const handleDelete = async (id) => {
    ...
    if (isConfirmed) {
      try {
        const response = await axios.delete(`http://localhost:8085/service/${id}`);
        if (response.status === 200) {
  ...
  const generatePDF = () => {
    ...
    // Logo setup (replace with actual Base64 string of your logo)
    ...
    try {
      ...
      alert("Logo failed to load in PDF. Please check the Base64 string.");
    }

    // Title & Metadata
    doc.setFontSize(18);
    doc.setTextColor(40, 40, 40);
    doc.text("All Services Report", pageWidth / 2, logoY + logoHeight + 10, {
      align: "center",
    });

    doc.setFontSize(12);
    doc.setTextColor(100);
    doc.text("Generated on: " + new Date().toLocaleDateString(), pageWidth / 2, logoY + logoHeight + 18, {
      align: "center",
    });

    // Table
    doc.autoTable({
      startY: logoY + logoHeight + 30,
    ...
    <div>
      ...
      <div className="page_with_set">
        ...
        <div className="admin_action_continer">
          <button className="pdf_btn" onClick={() => (window.location.href = "/addservice")}>
            Add Service
          </button>
          <input
          ...
          <tbody>
            {filteredServices.map((service) => (
              <tr key={service._id}>
                ...
                <td>{service.title}</td>
                <td style={{ whiteSpace: "pre-line" }}>{service.description}</td>
                <td style={{ whiteSpace: "pre-line" }}>{service.serviceSet}</td>
                <td style={{ whiteSpace: "pre-line" }}>${service.price}</td>
                <td className="btn_td_new">
                  <button className="btn_action" onClick={() => handleUpdate(service._id)}>
                    Update
                  </button>
                  <button className="deletbtn" onClick={() => handleDelete(service._id)}>
                    Delete
                  </button>
                </td>
              </tr>
