...
function AllInquiries() {
  ...
  const handleDelete = async (id) => {
    ...
    if (isConfirmed) {
      try {
        const response = await axios.delete(`http://localhost:8085/customer/${id}`);
        if (response.status === 200) {
  ...
  const generateReport = () => {
    ...
    try {
      ...
      alert("Failed to add the logo to the PDF.");
    }
    ...
    doc.text("Inquiries Report", pageWidth / 2, logoY + logoHeight + 10, { align: "center" });
    ...
    // Table
    doc.autoTable({
      head: [["Inquiry ID", "Full Name", "Email", "Subject", "Message", "Reply"]],
          ...
          <tbody>
            {filteredInquiries.map((inquiry) => (
              <tr key={inquiry._id}>
                ...
                <td style={{ whiteSpace: "pre-line" }}>{inquiry.inquirieMsg}</td>
                <td
                  style={{
                ...
                <td>
                  <button className="btn_action" onClick={() => handleUpdate(inquiry._id)}>
                    Reply
                  </button>
                  <button className="deletbtn" onClick={() => handleDelete(inquiry._id)}>
                    Delete
                  </button>
                </td>
              </tr>
